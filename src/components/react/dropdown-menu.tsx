...
function useMenuAnimation(isOpen: boolean) {
  ...
  useEffect(() => {
    ...
    animate(
      'ul',
      {
        clipPath: isOpen ? 'inset(0% 0% 0% 0% round 12px)' : 'inset(10% 50% 90% 50% round 12px)'
      },
      {
        type: 'spring',
        bounce: 0,
...
export function DropdownMenu({ containerClassName, itemClassName }: DropdownMenuProps) {
  ...
  return (
    <nav className={cn('max-w-[200px] w-full mx-auto space-y-2', containerClassName)} ref={scope}>
      <motion.button
        whileTap={{ scale: 0.97 }}
      ...
      >
        {items.map(({ icon, name, href, customStyle }) => (
          <li key={name}>
            <a
              href={href} // Where you will be sent
              className={cn(
